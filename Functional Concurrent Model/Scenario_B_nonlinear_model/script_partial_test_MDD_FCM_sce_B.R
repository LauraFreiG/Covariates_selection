...
fx1=5*sin(t*pi/12)
fx2=-(t-20)^2/50-4
f1_fun<-function(t,x){exp((t+1)*x/20)-2}
f2_fun<-function(t,x){-1.2*log(x^2)*sin(2*pi*t/24)}
...
for(m in 1:M){
    ...
    for(t_ind in 1:length(t)){
      ...
      S1= sqrt( (2/(n*(n-1)*cn))*suma_1 )
      S2= sqrt( (2/(n*(n-1)*cn))*suma_2 )
      
      Tn_1=sqrt(choose(n,2))*MDD[1]
      Tn_2=sqrt(choose(n,2))*MDD[2]
      
      S_vec_1=c(S_vec_1,S1)
      S_vec_2=c(S_vec_2,S2)
      ...
      for(b in 1:B_boot_n){
        ...
        S_boot_1=c(S_boot_1,sqrt(suma_1/choose(n,2)))
        S_boot_2=c(S_boot_2,sqrt(suma_2/choose(n,2)))
        Tn_boot_1=c(Tn_boot_1, sqrt(choose(n,2))*MDD_boot_1 )
        Tn_boot_2=c(Tn_boot_2, sqrt(choose(n,2))*MDD_boot_2 )
        
      }
      
      
      S_boot_mat_1=cbind(S_boot_mat_1,S_boot_1)
      S_boot_mat_2=cbind(S_boot_mat_2,S_boot_2)
    ...
    S_1=int.simpson2(x=t,y=S_vec_1,method="CSR")
    Tn_1=int.simpson2(x=t,y=Tn_vec_1,method="CSR")
    Tn_1=Tn_1/S_1
    S_2=int.simpson2(x=t,y=S_vec_2,method="CSR")
    Tn_2=int.simpson2(x=t,y=Tn_vec_2,method="CSR")
    Tn_2=Tn_2/S_2
    
    S_boot_1=apply(S_boot_mat_1,1,
                    function(z){int.simpson2(x=t,y=z,method="CSR")} )
    Tn_boot_1=apply(Tn_boot_mat_1,1,
                  function(z){int.simpson2(x=t,y=z,method="CSR")} )
    Tn_boot_1=Tn_boot_1/S_boot_1
    S_boot_2=apply(S_boot_mat_2,1,
                   function(z){int.simpson2(x=t,y=z,method="CSR")} )
    Tn_boot_2=apply(Tn_boot_mat_2,1,
                    function(z){int.simpson2(x=t,y=z,method="CSR")} )
    Tn_boot_2=Tn_boot_2/S_boot_2
