...
# testing=FALSE #if there is some training/testing distinction
# library(hdi)
# data(riboflavin)
# datos=riboflavin; rm(riboflavin)
# y=datos[,1]
# X=as.matrix(datos[,2]) # 71 x 4088
# n=dim(X)[1]
# p=dim(X)[2]
# nam_cov=colnames(X)
# x=matrix(X,n,p)
# y=y-mean(y)
# X=X-matrix(colMeans(X),71,4088,byrow=TRUE)
# if(stand=="yes"){
...
# Prostate cancer
stand="yes" # applying univariate standardization
testing=TRUE #if there is some training/testing distinction
data=read.table("Prostate.txt", header=T, sep="\t", dec=".")
dat_train=data[data$train==T,-c(10)] #training data
dat_test=data[data$train==F,-c(10)] #testing data
nam_cov=names(dat_train)
#using training data
x_est=dat_train[,-9]
y=dat_train[,9]
y=y-mean(y)
n=dim(x_est)[1]
p=dim(x_est)[2]
x_est=x_est-matrix(colMeans(x_est),n,p,byrow=TRUE)
x_est=as.matrix(x_est)
if(stand=="yes"){
  x_est=apply(x_est,2,function(z){z/sd(z)})
}
x=x_est
#using testing data
x_test=dat_test[,-9]
x_test=as.matrix(x_test)
x_test=x_test-matrix(colMeans(x_test),dim(x_test)[1],dim(x_test)[2],byrow=TRUE)
if(stand=="yes"){
  x_test=apply(x_test,2,function(z){z/sd(z)})
}
y_test=dat_test[,9]
...
# testing=FALSE #if there is some training/testing distinction
# library(readr)
# dat=read_csv("bodyfat-out.csv") #cleaning Body Fat data
# dat=dat[,-1]
# x=as.matrix(dat[,-15])
# y=as.vector(dat[,15]$BodyFat)
# y=y-mean(y)
# n=dim(x)[1]
# p=dim(x)[2]
...
# stand="yes" # applying univariate standardization
# testing=FALSE #if there is some training/testing distinction
# dat=read.csv("winequality-red-out.csv", header=T, sep=",",dec=".")
# dat=dat[,-1]
# x=dat[,-11]
# y=dat[,11]
# y=y-mean(y)
...
if(length(ind)==0){
  MSE[1]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[1]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[1]=RSS/n
...
if(length(ind)==0){
  MSE[2]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[2]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[2]=RSS/n
...
if(length(ind)==0){
  MSE[3]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[3]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[3]=RSS/n
...
if(length(ind)==0){
  MSE[4]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[4]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[4]=RSS/n
...
if(length(ind)==0){
  MSE[5]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[5]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[5]=RSS/n
...
if(length(ind)==0){
  MSE[6]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[6]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[6]=RSS/n
...
if(length(ind)==0){
  MSE[7]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[7]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[7]=RSS/n
...
if(length(ind)==0){
  MSE[8]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[8]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[8]=RSS/n
...
if(length(ind)==0){
  MSE[9]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[9]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[9]=RSS/n
...
if(length(ind)==0){
  MSE[10]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[10]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[10]=RSS/n
...
if(length(ind)==0){
  MSE[11]=ifelse(testing,sum(y_test^2)/n,sum(y^2)/n)
  pdev[11]=0
}else{
  RSS=ifelse(testing,sum((y_test-predict(lm(y_test~x_test[,ind])))^2),sum((y-predict(lm(y~x[,ind])))^2))
  MSE[11]=RSS/n
