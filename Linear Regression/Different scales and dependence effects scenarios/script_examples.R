...
# source("scenarios/IND.R")
# n=50 # sample size (n=25,50,100,150,300)
# scen="RC.IND" # scenario (scen="IND","RC.IND","RNC.IND")
# data=data_gener(n,scen)

# # Scenario 2: UTOEP
# stand="yes" # applying univariate standardization
# source("scenarios/UTOEP.R")
# n=50 # sample size (n=25,50,100,150,300)
# rho=0.5 # rho value (rho=0.5,0.9)
# scen="UTOEP-B" # scenario (scen="UTOEP-B","UTOEP-S")
# data=data_gener(n,rho,scen)

# Scenario 3
stand="no" # applying univariate standardization
source("scenarios/TOEP-S.R")
n=50 # sample size (n=25,50,100,150,300)
rho=0.5 # rho value (rho=0.5,0.9)
scen="RNC.TOEP-S" # scenario (scen="RC.TOEP-S","RNC.TOEP-S")
data=data_gener(n,rho,scen)
...
w3 <- 1/abs(matrix(coef(cv.ridge, s=cv.ridge$lambda.min)[, 1][2:(100+1)] ))^1
...
cv.ridge <- cv.glmnet(x,y,family="gaussian", alpha=0, parallel=F, standardize=FALSE,
                      standardize.response=FALSE, intercept=FALSE, nfold=nfold)
w3 <- 1/abs(matrix(coef(cv.ridge, s=cv.ridge$lambda.1se)[, 1][2:(100+1)] ))^1
w3[w3[,1] == Inf] <- 999999999 ## Replacing values estimated as Infinite for 999999999
beta=rep(0,100)
iter=TRUE
while(iter==TRUE){
...
#results in a matrix
Results=cbind(sel,round(MSE,3),round(pdev,3))
rownames(Results)=names(cov_sel); colnames(Results)=c("S_hat","MSE","%Dev")
Results
